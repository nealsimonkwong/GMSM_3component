%% Get multicomponent CMS given A_H at specified T*
...
Tstar = 0.75; % Example periods: 0.1 0.5 0.75
...
Atgt = exp(interp1( log(Tuhs), log(UHScurr_H), log(Tstar) )); % Depends on conditioning component of GM

% Determine GMPM output for all rupture scenarios
MUs_Tstar = MUs_geo_all(:,idTstar); % Depends on conditioning component of GM
SIGs_Tstar = SIGs_geo_all(:,idTstar); % Depends on conditioning component of GM
...
rhoHVHTstarT = zeros(1,nT); rhoHVHsameT = zeros(1,nT);
for ii=1:nT
%     [VHratio(1,ii), sVH(1,ii)] = GA_2011(Mbar,Rbar,Vs30,FRV,FNM,Tcms(ii));
    [VHratio(1,ii), sVH(1,ii)] = gmpmVoverH_BC2016(Mbar,Rbar,Rbar,Rbar,lambda,dip,Fhw,Vs30,region,Sj,Tcms(ii),W,Ztor,Z2p5,Zhyp);       
    rhoHVHTstarT(1,ii) = GA_2011_corr(Tstar, Tcms(ii), Mbar, Rbar, Vs30, FRV, FNM); % Note: rho_H_V/H(T,T*) not the same as rho_H_V/H(T*,T)
...
rhoHVTstarT_derived = (sH.*rhoHHTstarT + sVH.*rhoHVHTstarT) ./ sV_derived; % Order of input periods matter for rhoHV

%% Compute CMS - Epsilon at conditioning component of GM and T*
% Epsilon at T*
Am_Tstar = AmH(1,idTstar); % Depends on conditioning component of GM
sig_Tstar = sH(1,idTstar); % Depends on conditioning component of GM
epsTstar = (log(Atgt)-log(Am_Tstar)) / sig_Tstar;

%% Compute CMS - target median and dispersion
% CMS_H
CMS_H = AmH .* exp( sH .* (epsTstar * rhoHHTstarT) ); % Depends on conditioning component of GM

% CMS_V
CMS_V = AmV .* exp( sV .* (epsTstar * rhoHVTstarT_derived) ); % Depends on conditioning component of GM

% Conditional standard deviations
CMSsig_H = sH.*sqrt(1-rhoHHTstarT.^2); % Depends on conditioning component of GM
CMSsig_V = sV.*sqrt(1-rhoHVTstarT_derived.^2); % Depends on conditioning component of GM
...
save(fullfile(outputDir,outputFilename),...
    ...
    'rhoHVHTstarT','rhoHVTstarT_derived',...
