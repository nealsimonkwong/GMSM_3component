%% Multicomponent Composite Spectrum given Av at Tmin to Tmax
...
Atgt = UHScurr_V(1,idTbar); % Depends on conditioning component of GM

% Determine GMPM output for all rupture scenarios
MUs_Tbar = MUs_V_all(:,idTbar); % Depends on conditioning component of GM
SIGs_Tbar = SIGs_V_all(:,idTbar); % Depends on conditioning component of GM
...
% Epsilon at Tbar
epsV = (log(UHScurr_V(idTbar) ./ Av(idTbar))) ./ sigV(idTbar); % Depends on conditioning component of GM

% Target spectrum
UHSbarH = Ah .* exp( epsV .* rho_H_V_sameT_derived .* sigH ); % Depends on conditioning component of GM
...
CompSpec_H(idMed) = UHSbarH( 1 , idMed ); % Depends on conditioning component of GM
CompSpec_H(idLong) = CMSs_H( 2 , idLong );

% V
CompSpec_V = zeros(1,nT);
CompSpec_V(idShort) = CMSs_V( 1 , idShort );
CompSpec_V(idMed) = UHScurr_V( 1 , idMed ); % Depends on conditioning component of GM
CompSpec_V(idLong) = CMSs_V( 2 , idLong );

%% Save data
save(fullfile(outputDir,outputFilename),...
    'Tmin','Tmax','Tcms',...
    'TbarExact','Tbar','UHSbarH',...
