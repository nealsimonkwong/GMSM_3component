...
function [Sa, tsigma, period1, pga_rock, sigma, tau] = AS_2008_nga_mod_wCD(M, Vs30, T, Rrup, Rjb, Rx, dip, Ztor, Z10, W, FRV, FNM, FAS, FHW, FVS30)
...
% %         if ~ismember(Teach,period); 
        if all(Teach ~= period) % If current period not one of those considered in GMPM; use logical indexing 
% %         if all( abs(period-Teach) > 1e-4 ) % If current period not one of those considered in GMPM; use logical indexing 
