...
% Modified 6/11/2018 by Neal Simon Kwong to export tau, phi, and update c6
% coefficients for long periods
%
...
function [Sa, sigma, period1, tau, phi] = CB_2014_nga(M, T, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, lambda, Fhw, Vs30, Z25, Zhyp, region)
...
if length (T) == 1 && T == 1000; 
    ...
    tau=zeros(1,length(period)-2);
    phi=zeros(1,length(period)-2);
    period1=period(1:end-2);
   
    for ipT=1:length(period1)
        [Sa(ipT),sigma(ipT),tau(ipT),phi(ipT)]=CB_2014_nga_sub (M, ipT, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region, A1100);
...
% Compute Sa and sigma with user-defined period    
    ...
    for i=1:length(T)
        ...
        if ( isempty(find(abs(period-Ti) < 0.0001, 1))) % The user defined period requires interpolation
            ...
            [Sa_low, sigma_low, tau_low, phi_low] = CB_2014_nga_sub (M, ip_low, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region, A1100);
            [Sa_high, sigma_high, tau_high, phi_high] = CB_2014_nga_sub (M, ip_high, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region, A1100);
            [PGA,~]=CB_2014_nga_sub (M, 22, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region, A1100);
            ...
            [Sa(i), sigma(i), tau(i), phi(i)] = CB_2014_nga_sub (M, ip_T, Rrup, Rjb, Rx, W, Ztor,Zbot,delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region,A1100);
            [PGA,~]=CB_2014_nga_sub (M, 22, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lambda, Frv, Fnm, region, A1100);
