...
function [rho_total, rho_between, rho_within] = GA_2011_corr(T_H, T_VH, M, Rrup, Vs30, FRV, FNM)
%{
...
Provides correlations between SA_H(T_H) and SA_VH(T_VH)

Input Variables
T_H          = Period for horizontal component (sec); from 0.01 to 10 sec
T_VH         = Period for V/H component (sec); from 0.01 to 10 sec
M            = Magnitude
...
% %         T_H_lo = T_GA2011(find(T_GA2011<=T_H,1,'last'));
% %         T_H_up = T_GA2011(find(T_GA2011>=T_H,1,'first'));
% %         T_VH_lo = T_GA2011(find(T_GA2011<=T_VH,1,'last'));
% %         T_VH_up = T_GA2011(find(T_GA2011>=T_VH,1,'first'));
...
% %                     V(ii,jj) = GA_2011_corr_fixedT(xx,yy,M,Rrup,Vs30,FRV,FNM);
                end
            end
            % Linearly interpolate
            % % %         rho_total = interp2(X,Y,V,T_H,T_VH);
...
% Assume vertical SS for now
Rjb = Rrup;
Rx = Rrup;
dip = 90; 
Ztor = 0; % Assume motion at top; better way to assume?
Z10 = 0; % Value here doesn't matter if SS assumed; better way to assume?
W = 15; % Value here doesn't matter if SS assumed; better way to assume?
FAS = 0;
FHW = 0;
FVS30 = 0;
...
%% Sigmas from GMPMs
% Std dev for H
[~, sig_H, ~, ~, phi_H, tau_H] = AS_2008_nga_mod_wCD(M, Vs30, T_H, Rrup, Rjb, Rx, dip, Ztor, Z10, W, FRV, FNM, FAS, FHW, FVS30);

% Std dev for V/H
[~, sig_VH, phi_VH, tau_VH] = GA_2011(M,Rrup,Vs30,FRV,FNM,T_VH);
